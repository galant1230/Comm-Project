...
- 消掉不同係數之間的「**尺度差異**」
- 圖中比「未處理」準確率高一點（ $62.65 \rightarrow 67.9$ ）
...
一段音檔做完 MFCC 之後，可以想成一個矩陣（表）：

- 橫的：時間 frame（第 1 幀、第 2 幀…）
- 直的：特徵維度（第 1 個 MFCC 係數、第 2 個 MFCC 係數…）
...
- 第 $t$ 列 = 第 $t$ 個時間點的所有特徵  
- 第 $d$ 欄 = 第 $d$ 個 MFCC 係數，隨時間變化的一條曲線  

---

## 「對特徵」標準化 = 對欄 (column) 做事

也就是：**每一個特徵維度，自己單獨算平均、標準差**。

對第 $d$ 個特徵（第 $d$ 欄）：

- 看所有時間點 $t = 1 \dots T$ 的值 $x_{t,d}$  
- 算這一欄的平均和標準差：

$$
\mu_d = \frac{1}{T} \sum_{t=1}^{T} x_{t,d}, \qquad
\sigma_d = \sqrt{\frac{1}{T} \sum_{t=1}^{T} (x_{t,d} - \mu_d)^2}
$$
...
- 每一個 MFCC 係數（每一欄）都被「單獨拉直」到 **零均值、單位變異數**
- 但同一個時間點那一列裡，各維之間的相對大小關係還在  
  → 頻譜包絡的「形狀」不會被破壞  

你可以想成：  
> 「我把每條特徵曲線拉到差不多的 scale，好訓練模型。」

---

## 「對時間」標準化 = 對列 (row) 做事

這次是：**每一個時間點，把當下所有特徵一起標準化**。

對第 $t$ 個時間（第 $t$ 列）：

- 看這一幀所有特徵 $x_{t,1}, x_{t,2}, \dots, x_{t,D}$  
- 算這一列的平均和標準差：

$$
\mu_t = \frac{1}{D} \sum_{d=1}^{D} x_{t,d}, \qquad
\sigma_t = \sqrt{\frac{1}{D} \sum_{d=1}^{D} (x_{t,d} - \mu_t)^2}
$$
...
- 每一個時間點的那一列，被強迫：
  - 平均變成 $0$
  - 變異數變成 $1$
- 也就是：**同一幀裡各維之間的「高、低、凸、凹」都被壓扁了**

而這些「高、低、凸、凹」其實就是頻譜包絡的形狀，  
跟音色 / 共振峰有關，是很重要的資訊。  
所以對時間標準化容易把有用資訊一起洗掉。
